  ...
  $(function () {
    $(".lazy").Lazy({
      ...
      afterLoad: function (e) {
        e.removeClass("lazy").addClass("lazyLoaded"), mansoryInit();
      },
    });
  });
}
...
function responsiveSlider() {
  $(window).on("load resize", function () {
    $(window).width() < 1200 &&
      ($(".engineer-slider:not(.slick-initialized)").length > 0 &&
        $(".engineer-slider:not(.slick-initialized)").slick(),
      $(".skill-list-slider:not(.slick-initialized)").length > 0 &&
        $(".skill-list-slider:not(.slick-initialized)").slick());
  });
}
...
function stickyHeder() {
  function e(e) {
    e > n
      ...
      : (i.removeClass("is-scroll"), $(".clone-nav").show()),
      e > 0
  ...
  function t() {
    (i = $(".header-wrap")),
      ...
      (n = ($(window).width() <= 992 || $(window).height(), 49)),
      e(a, n);
  }
  if (void 0 !== $(".header-wrap")) {
    var i = $(".header-wrap"),
      a = $(window).scrollTop(),
      n = i.offset().top;
    n = ($(window).width() <= 992 || $(window).height(), 25);
...
function menuHover() {
  ...
  if (e.length > 0) {
    var t,
      i,
      a = $(".main-nav .menu>li.active").position().left,
      n = $(".main-nav .menu>li.active").width();
    e.css({ left: a + "px", width: n + "px" }),
      $(".main-nav .menu>li").on("mouseover", function () {
        (t = $(this).position().left),
          (i = $(this).width()),
          e.css({ left: t + "px", width: i + "px" });
      }),
...
function iniPlayer() {
  ...
  $(".popup-player-link").on("click", function () {
    ...
    return a.loadMedia(e - 1), !1;
  });
}
function tiltInit() {
  $(".js-tilt").tilt({ perspective: 1e3, speed: 50 });
}
function inputMask() {
...
function callPopup() {
  $(".open-popup-link").on("click", function (e) {
    ...
    var t = $(this),
      i = t.attr("href"),
      a = t.attr("data-href");
    $(i).addClass("show"), $(a).addClass("show");
  }),
    ...
    $(document).mouseup(function (e) {
      ...
      t.is(e.target) ||
        ...
        t.closest(".popup-wrap").removeClass("show");
    });
}
function textSlider() {
  function e(e) {
  ...
  function i(e) {
    ...
    if (e.parents(".cd-headline").hasClass("letters")) {
      ...
      a(e.find("i").eq(0), e, c, r), n(t.find("i").eq(0), t, c, r);
    } else
      s(e, t),
        setTimeout(function () {
          i(t);
  ...
  var l = 2500,
    r = 50;
  !(function () {
    e($(".cd-headline").find("li")), t($(".cd-headline"));
  })();
}
...
function tabInit() {
  $(".tab-links__item").on("click", function () {
    ...
    e.closest(".tab-wrap").find(".tab-links__item").removeClass("active"),
      ...
      e
        .closest(".tab-wrap")
        .find(".tab-blocks__item[data-show-tab=" + t + "]")
        .addClass("active");
  });
}
function clientFilterInit() {
  $("[data-for-filter] a").on("click", function (e) {
    ...
    a.find("a").removeClass("active"),
      t.addClass("active"),
      "*" == i || "all" == i
        ? $(n).find(".filter-elem").slideDown()
        : ($(n).find(".filter-elem").slideUp(),
          $(n)
            .find("." + i)
            .slideDown());
  });
...
function gridFilter() {
  $(".massonry-filter-list a").on("click", function (e) {
    ...
    $(this)
      .closest(".massonry-grid-wrap")
      .find(".massonry-grid")
      .find(".gallery-grid__item"),
...
function initMap() {
  function e(e) {
    ...
    for (
      var r = [{ coordinates: { lat: n, lng: o }, myMap: i, iconImg: s }],
        c = 0;
      c < r.length;
      c++
    )
...
window.onscroll = function() {
  ...
  if (prevScrollpos > currentScrollPos) {
    ...
    // document.getElementsByClassName("main-nav")[0].style.top = "-100px";
    document.getElementsByClassName("header-wrap")[0].style.top = "-100px";
